...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calor : float = ((self.coeff_calories_1 * self.get_mean_speed()
...
class SportsWalking(Training):
    ...
    coeff_calories_1 : int = 0.035
    coeff_calories_2 : int = 0.029
    def __init__(self, 
                 action: int,
                 duration: float,
                 weight: float,
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calor : float = ((self.coeff_calories_1 * self.weight +
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        calor : float = ((self.get_mean_speed() + self.coeff_calories_1)
                        * 2 * self.weight)
        return calor
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    naming = {'SWM': Swimming,
            'RUN':Running,
            'WLK':SportsWalking}
    class_action = naming[workout_type]
    return class_action(*data)
